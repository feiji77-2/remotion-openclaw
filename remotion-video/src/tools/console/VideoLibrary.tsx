...
export const VideoLibrary: React.FC<VideoLibraryProps> = ({onBack, onOpenProject}) => {
  ...
  useEffect(() => {
    let mounted = true;
    void loadVideoLibrary().then((items) => {if (mounted) applyRecords(items);}).finally(() => {if (mounted) setLoading(false);});
    return () => {mounted = false;};
  }, []);

  return <section className="library-screen">
    <header className="library-header"><div><button type="button" className="text-action" onClick={onBack}>← 工作台</button><span className="workspace-kicker">VIDEO LIBRARY</span><h1>视频库</h1><p>已完成渲染的成片会在这里保留。只有确认可交付的版本可以下载。</p></div><button type="button" className="secondary-action" onClick={() => void loadVideoLibrary().then(applyRecords)}>刷新</button></header>
    {loading && <div className="library-empty">正在读取成片记录</div>}
    {!loading && records.length === 0 && <div className="library-empty"><strong>还没有成片</strong><span>完成一次渲染后，成片会自动出现在这里。</span></div>}
