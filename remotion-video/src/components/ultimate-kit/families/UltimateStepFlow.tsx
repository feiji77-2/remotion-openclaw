...
export const UltimateStepFlow: React.FC<UltimateStepFlowProps & {grammar?: {staggerGap?: number}}> = ({
  ...
  const pointPresets = [
    {x: 218, y: 620, labelX: -10, labelY: -134, align: 'left' as const},
    {x: 548, y: 458, labelX: -20, labelY: -142, align: 'left' as const},
    {x: 942, y: 636, labelX: -100, labelY: -150, align: 'center' as const},
    {x: 1332, y: 446, labelX: -240, labelY: -142, align: 'right' as const},
    {x: 1696, y: 624, labelX: -250, labelY: -140, align: 'right' as const},
  ];
  const sampleCubic = (
    t: number,
  ...
  const segments = visibleSteps.slice(0, -1).map((step, index) => {
    const from = pointPresets[index];
    const to = pointPresets[index + 1];
    const p0 = {x: from.x, y: from.y};
    const p1 = {x: from.x + 128, y: from.y + (index % 2 === 0 ? -18 : 22)};
    const p2 = {x: to.x - 128, y: to.y + (index % 2 === 0 ? -22 : 18)};
    ...
    <div style={{position: 'absolute', inset: 0, padding: `${kit.spacing.pageY}px ${kit.spacing.pageX}px`}}>
      ...
      <div
        style={{
          ...
          borderRadius: 44,
          overflow: 'hidden',
          background: 'linear-gradient(180deg, rgba(6, 9, 18, 0.72), rgba(6, 9, 18, 0.54))',
      ...
      >
        ...
        <svg viewBox="0 0 1720 720" style={{position: 'absolute', inset: 0, width: '100%', height: '100%', overflow: 'visible'}}>
          {segments.map((segment, index) => {
            const color = toneToColor(visibleSteps[index].accent ?? (index % 2 === 0 ? 'cyan' : 'green'));
        ...
        {visibleSteps.map((step, index) => {
          ...
          const accentColor = toneToColor(step.accent ?? (index % 2 === 0 ? 'cyan' : 'green'));
          const delay = 10 + index * gap * 5;
          const reveal = buildReveal(frame, Math.max(0, delay - 8));
