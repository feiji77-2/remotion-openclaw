...
const HERO_STAGE_HEIGHT =
  storyboardContract.zones.hero.bottom - storyboardContract.zones.hero.top;
const HERO_STAGE_SCALE = 1;
const SEMANTIC_STAGE_TOP = storyboardContract.zones.semantic.top;
const SEMANTIC_STAGE_HEIGHT =
  storyboardContract.zones.semantic.bottom -
  storyboardContract.zones.semantic.top;
const clamp = {
...
const layerOpacity = (frame: number, beat: SkillShowcaseBeat) => {
  ...
  const enter = interpolate(
    frame,
    [beat.startFrame - overlap, beat.startFrame + 10],
    [0, 1],
    { ...clamp, easing: easeOut },
  );
  const exit = interpolate(
    frame,
    [beat.endFrame - 9, beat.endFrame + overlap],
    [1, 0],
    { ...clamp, easing: Easing.in(Easing.cubic) },
  );
  return enter * exit;
};
const lift = (progress: number, distance = 42) =>
    ...
    <>
      <div
        style={{
          ...
          top: 42,
          display: "grid",
          gridTemplateColumns: `repeat(${progressTotal}, 1fr)`,
          gap: 7,
        }}
      >
        {Array.from({ length: progressTotal }).map((_, itemIndex) => (
          <div
            key={itemIndex}
            style={{
              height: itemIndex === progressIndex ? 5 : 3,
      ...
      <div
        style={{
          ...
          gap: 16,
          opacity: p,
          transform: `translateY(${lift(p, -18)}px)`,
        }}
      >
        {index ? (
          <div
...
const StageTitle: React.FC<{
  ...
  progress: number;
  align?: "left" | "center";
}> = ({ beat, accent, progress, align = "center" }) => (
  <div
    style={{
      position: "absolute",
      left: 56,
      ...
      textAlign: align,
      opacity: progress,
      transform: `translateY(${lift(progress, 30)}px)`,
    }}
  >
    <div
      style={{
    ...
    >
      VOICE HIT / {beat.action.toUpperCase()}
    </div>
    <div
      style={{
        fontSize: beat.keyword.length > 9 ? 54 : 68,
        lineHeight: 1.02,
        color: "#fff",
        fontWeight: 950,
        marginTop: 14,
...
const PhoneInterface: React.FC<{
  ...
  const background = purple
    ? "linear-gradient(150deg, #6d39b5, #263d9d)"
    : mono
      ? "#f4f1e8"
      : PORTRAIT_COLOR_THEME.surface;
  ...
  return (
    <div
      style={{
        ...
        height,
        borderRadius: 30,
        border: `2px solid ${purple ? "#a777ff" : accent}`,
        background,
        overflow: "hidden",
        padding: compact ? 24 : 30,
        boxShadow: `0 28px 80px rgba(0,0,0,0.45), 0 0 42px ${accent}24`,
        color: foreground,
      }}
    >
      <div
        style={{
          ...
          borderRadius: 20,
          margin: "0 auto",
          background: mono ? "#111" : "rgba(255,255,255,0.78)",
        }}
      />
      <div
        style={{
          ...
          fontSize: 11,
          letterSpacing: 1.4,
          color: purple ? "#f7e9ff" : mono ? "#444" : accent,
        }}
      >
        SYSTEM / LIVE OUTPUT
      </div>
      ...
      >
        {purple ? "Everything, centered." : "A clear point of view."}
      </div>
      <div
        style={{
          height: 5,
          width: `${interpolate(progress, [0, 1], [18, 72])}%`,
          background: purple ? "#d9b8ff" : accent,
          marginTop: 24,
        }}
      />
      {[0, 1, 2, 3].map((index) => (
        <div
          ...
          style={{
            ...
            border: `1px solid ${purple ? "rgba(255,255,255,0.16)" : mono ? "#bbb7ad" : `${accent}35`}`,
            transform: `translateX(${interpolate(progress, [0, 1], [index % 2 ? 40 : -40, 0])}px)`,
          }}
        />
      ))}
      <div
        style={{
          ...
          bottom: 28,
          height: 54,
          borderRadius: purple ? 28 : 4,
          display: "grid",
          placeItems: "center",
          background: purple ? "#efe9ff" : accent,
          color: purple ? "#6c35b4" : "#07100b",
          fontWeight: 950,
          fontSize: 15,
        }}
      >
      ...
      {scanY !== undefined ? (
        <div
          style={{
            ...
            top: scanY,
            height: 5,
            background: `linear-gradient(90deg, transparent, #fff, ${secondary}, transparent)`,
            boxShadow: `0 0 28px ${secondary}`,
          }}
        />
      ) : null}
    </div>
...
const KineticTypeShot: React.FC<{
  frame: number;
  beat: SkillShowcaseBeat;
  accent: string;
  secondary: string;
  opacity: number;
}> = ({ frame, beat, accent, secondary, opacity }) => {
  const p = phase(frame, beat);
  const state = beat.visualState ?? "";
  const tokens = beat.evidence?.length
    ? beat.evidence
    : ["TYPE", "COLOR", "GRID", "SPACE"];
  ...
  return (
    <AbsoluteFill style={{ opacity }}>
      <StageTitle
        beat={beat}
        accent={accent}
        progress={Math.min(1, p * 5)}
        align={metrics ? "center" : "left"}
      />
      {metrics ? (
        <div
          style={{
            ...
            return (
              <div
                ...
                style={{
                  display: "grid",
                  placeItems: "center",
                  borderTop: `3px solid ${PALETTE[index]}`,
                  color: PALETTE[index],
                  fontSize: 116,
                  fontWeight: 950,
                  transform: `translate(${interpolate(enter, [0, 1], [x, 0])}px, ${interpolate(enter, [0, 1], [y, 0])}px) scale(${interpolate(enter, [0, 1], [1.4, 1])})`,
                  opacity: enter,
                }}
              >
                {value}
              ...
              return (
                <div
                  ...
                  style={{
                    position: "absolute",
                    left: 40 + (index % 2) * 430,
                    top: 80 + Math.floor(index / 2) * 250,
                    width: 390,
                    fontSize: anti ? 42 : 54,
                    fontWeight: 950,
                    color: anti ? "#ff718b" : index % 2 ? secondary : "#fff",
                    transform: `translate(${interpolate(enter, [0, 1], [scatterX, 0])}px, ${interpolate(enter, [0, 1], [scatterY, 0])}px) rotate(${interpolate(enter, [0, 1], [index % 2 ? 14 : -12, 0])}deg)`,
                    opacity: enter,
                  }}
                >
                  {token}
                  <div
                    style={{
                      height: anti ? 6 : 3,
                      width: `${interpolate(enter, [0, 1], [0, anti ? 100 : 72])}%`,
                      background: anti ? "#ff4f70" : accent,
                      marginTop: 12,
                      transform: anti
...
const SplitWipeShot: React.FC<{
  ...
  const p = phase(frame, beat);
  const reveal = interpolate(p, [0.08, 0.68], [0, 1], {
    ...clamp,
    easing: easeOut,
  });
      ...
      >
        <div
          style={{
            ...
            background: pain
              ...
              : "linear-gradient(150deg, rgba(111,58,183,0.8), rgba(39,59,148,0.68))",
            display: "flex",
        ...
        <div
          style={{
            position: "absolute",
            inset: 0,
            clipPath: `inset(0 0 ${Math.max(0, 100 - reveal * 100)}% 0)`,
            background: pain
              ? `radial-gradient(circle at 50% 42%, ${accent}25, rgba(7,10,18,0.98) 66%)`
              : "rgba(7,10,18,0.97)",
            display: "flex",
            flexDirection: "column",
            alignItems: "center",
        ...
        >
          {pain ? (
            <svg width="720" height="720" viewBox="0 0 720 720">
              <path
                d="M84 560 C150 210 300 520 368 180 C440 12 590 250 646 96"
                fill="none"
                stroke={accent}
                strokeWidth="5"
        ...
        <div
          style={{
            ...
            top: dividerY,
            height: 5,
            background: `linear-gradient(90deg, transparent, ${accent}, #fff, ${secondary}, transparent)`,
...
const ParticleFieldShot: React.FC<{
  ...
  const labels = brandMode
    ...
    : categoryMode
      ? ["字体", "色彩", "布局", "间距", "网格", "对比", "层级", "无障碍"]
      : [];
  const centerText =
    beat.value ?? (brandMode ? "68" : categoryMode ? "8" : "22K");
  return (
    <AbsoluteFill style={{ opacity }}>
      <StageTitle beat={beat} accent={accent} progress={Math.min(1, p * 5)} />
      <div
      ...
      >
        {Array.from({ length: 42 }).map((_, index) => {
          const angle =
            seeded(index + 2) * Math.PI * 2 +
            frame * (index % 2 ? 0.002 : -0.0015);
          ...
          return (
            <div
              ...
              style={{
                ...
                background:
                  index % 5 === 0 ? "#fff" : PALETTE[index % PALETTE.length],
                boxShadow: `0 0 ${size * 3}px ${PALETTE[index % PALETTE.length]}`,
                opacity: 0.34 + seeded(index) * 0.66,
              }}
            />
          );
        })}
        <div
          style={{
            ...
            left: "50%",
            top: 520,
            transform: `translate(-50%, -50%) scale(${interpolate(Math.min(1, p * 3), [0, 1], [1.8, 1])})`,
            fontSize: centerText.length > 3 ? 146 : 210,
            lineHeight: 0.8,
            color: accent,
            fontWeight: 950,
...
const OrbitalMapShot: React.FC<{
  ...
  const labels = database
    ? ["COLOR", "TYPE", "SPACE", "WCAG", "GRID", "VOICE"]
    : ["命名", "扫描", "对比", "解释", "布局", "色彩"];
      ...
      >
        ...
        >
          {[210, 310, 410].map((radius, index) => (
            <circle
              ...
              r={radius}
              fill="none"
              stroke={index === 1 ? accent : "rgba(255,255,255,0.13)"}
              strokeWidth={index === 1 ? 4 : 2}
              strokeDasharray={`${20 + index * 10} ${18 + index * 8}`}
              strokeDashoffset={(index % 2 ? 1 : -1) * frame * (2 + index)}
              opacity={0.45 + index * 0.15}
            />
          ))}
          {Array.from({ length: 16 }).map((_, index) => {
            const a =
              (index / 16) * Math.PI * 2 + frame * 0.003 * (index % 2 ? 1 : -1);
        ...
        <div
          style={{
            ...
            left: "50%",
            top: 430,
            transform: `translate(-50%, -50%) scale(${interpolate(Math.min(1, p * 4), [0, 1], [1.5, 1])})`,
            width: 270,
            height: 270,
            borderRadius: database ? 36 : "50%",
            border: `5px solid ${accent}`,
            background: "rgba(7,10,18,0.94)",
            display: "grid",
            placeItems: "center",
            color: accent,
            fontSize: core.length > 2 ? 90 : 132,
            lineHeight: 1,
            fontWeight: 950,
        ...
        {labels.map((label, index) => {
          ...
          return (
            <div
              ...
              style={{
                position: "absolute",
                left: 500 + Math.cos(angle) * radius,
                top: 430 + Math.sin(angle) * radius,
            ...
            return (
              <div
                ...
                style={{
                  ...
                  width: 58,
                  height: 58,
                  border: `3px solid ${index === 1 ? "#ff5f7a" : accent}`,
                  opacity: visible,
                  transform: `scale(${visible ? 1 : 1.8})`,
                  boxShadow: `0 0 30px ${index === 1 ? "#ff5f7a" : accent}66`,
                }}
              >
                <div
                  style={{
...
const MaterialCarouselShot: React.FC<{
  frame: number;
  beat: SkillShowcaseBeat;
  accent: string;
  opacity: number;
  ...
  const items = brands
    ? ["Stripe", "Linear", "Vercel", "Recta"]
    : ["Swiss", "RAW", "Nordic", "Neo", "Editorial", "Utility"];
  return (
    <AbsoluteFill style={{ opacity }}>
      <StageTitle beat={beat} accent={accent} progress={Math.min(1, p * 5)} />
      {gradient ? (
            ...
            return (
              <div
                ...
                style={{
                  ...
                  width: size,
                  height: size,
                  transform: `translate(-50%, -50%) rotate(${frame * (index % 2 ? 0.4 : -0.3)}deg)`,
                  borderRadius:
                    index % 2 ? "34% 66% 48% 52%" : "58% 42% 64% 36%",
                  background: `linear-gradient(${120 + index * 22}deg, ${index % 2 ? "#7c3aed" : "#4f46e5"}aa, ${index % 3 ? "#ec4899" : "#06b6d4"}44)`,
                  filter: "blur(1px)",
                  mixBlendMode: "screen",
                  opacity: 0.32 + index * 0.08,
                  boxShadow: "inset 0 0 70px rgba(255,255,255,0.12)",
                }}
              />
            );
          })}
          ...
          >
            {Array.from({ length: 6 }).map((_, index) => (
              <div
                ...
                style={{
                  height: 180,
                  borderLeft: `3px solid ${PALETTE[index]}`,
                  background: `${PALETTE[index]}0c`,
                  transform: `translateY(${interpolate(Math.min(1, p * 3 - index * 0.12), [0, 1], [120, 0], clamp)}px)`,
                }}
              />
            ))}
          </div>
        ...
        >
          {items.map((item, index) => {
            const spread = interpolate(
              Math.min(1, p * 3),
              [0, 1],
              [0, (index - (items.length - 1) / 2) * 86],
            );
            const active =
              Math.floor(p * items.length * 1.5) % items.length === index;
            ...
            return (
              <div
                ...
                style={{
                  position: "absolute",
                  left: 540,
                  top: 470 + Math.abs(index - items.length / 2) * 22,
                  width: 620,
                  height: 730,
                  transformOrigin: "50% 110%",
                  transform: `translate(-50%, -50%) translateX(${spread}px) rotate(${spread * 0.12}deg) translateZ(${active ? 90 : -index * 8}px)`,
                  background:
                    item === "Vercel"
                      ? "#f4f2eb"
                      : `linear-gradient(160deg, ${color}2a, rgba(9,12,21,0.98) 54%)`,
                  borderTop: `7px solid ${color}`,
                  boxShadow: active
                    ? `0 0 70px ${color}44`
                    : "0 28px 70px rgba(0,0,0,0.55)",
                  padding: 38,
                  color: item === "Vercel" ? "#111" : "#fff",
              ...
              >
                ...
                <div style={{ fontSize: 74, fontWeight: 950, marginTop: 28 }}>
                  {item}
                </div>
                <div
                  style={{
...
const FocusLockShot: React.FC<{
  ...
  const p = phase(frame, beat);
  const lock = interpolate(p, [0.05, 0.6], [0, 1], {
    ...clamp,
    easing: easeOut,
  });
    ...
    <AbsoluteFill style={{ opacity }}>
      ...
      <div
        style={{
          ...
          backgroundSize: `${interpolate(lock, [0, 1], [116, 72])}px ${interpolate(lock, [0, 1], [116, 72])}px`,
        }}
      >
        <div
          style={{
        ...
        >
          <div style={{ fontFamily: MONO, fontSize: 13, color: accent }}>
            SELECTED / SWISS
          </div>
          <div
            style={{
          ...
          <div
            style={{
              width: `${interpolate(lock, [0, 1], [22, 84])}%`,
              height: 7,
              background: accent,
              marginTop: 32,
            }}
        ...
        <div
          style={{
            ...
            width: 190,
            height: 190,
            transform: `translate(-50%, -50%) rotate(${interpolate(lock, [0, 1], [18, 0])}deg)`,
        ...
        <div
          style={{
            ...
            fontFamily: MONO,
            fontSize: 15,
            color: lock > 0.92 ? accent : "rgba(255,255,255,0.42)",
            letterSpacing: 2,
          }}
        >
          {lock > 0.92 ? "DIRECTION LOCKED" : "SEARCHING DIRECTION"}
        </div>
      </div>
    </AbsoluteFill>
  );
...
const PipelineFlowShot: React.FC<{
  frame: number;
  beat: SkillShowcaseBeat;
  accent: string;
  secondary: string;
      ...
      >
        <div
          style={{
            ...
            background: `linear-gradient(180deg, #ff5f7a, ${accent}, ${secondary})`,
          }}
        />
        <div
          style={{
            position: "absolute",
            left: "50%",
            top: interpolate((p * 1.35) % 1, [0, 1], [90, 930]),
            width: 24,
            height: 24,
            transform: "translate(-50%, -50%) rotate(45deg)",
            background: "#fff",
          ...
          return (
            <div
              ...
              style={{
                ...
                width: 420,
                height: 190,
                borderLeft: `6px solid ${index === 0 ? "#ff5f7a" : index === 1 ? accent : secondary}`,
                background: "rgba(10,14,24,0.88)",
                padding: 28,
                transform: `translateX(${interpolate(enter, [0, 1], [index % 2 ? 120 : -120, 0])}px)`,
                opacity: enter,
              }}
            >
              <div
                style={{
                  ...
                  fontSize: 12,
                  color:
                    index === 0 ? "#ff758e" : index === 1 ? accent : secondary,
                  letterSpacing: 1.4,
                }}
              >
                {code}
...
const TokenAssemblyShot: React.FC<{
  frame: number;
  beat: SkillShowcaseBeat;
  accent: string;
  secondary: string;
        ...
        {tokens.map((token, index) => {
          ...
          return (
            <div
              ...
              style={{
                position: "absolute",
                left: interpolate(enter, [0, 1], [fromX, targetX]),
                top: interpolate(enter, [0, 1], [fromY, targetY]),
                width: 190,
                height: 70,
                transform: `translate(-50%, -50%) scale(${interpolate(enter, [0, 1], [1.35, 1])})`,
                display: "grid",
                placeItems: "center",
                borderTop: `4px solid ${PALETTE[index]}`,
                background: "rgba(7,10,18,0.92)",
                color: "#fff",
                fontFamily: MONO,
                fontSize: 14,
                fontWeight: 900,
                boxShadow: `0 0 26px ${PALETTE[index]}33`,
              }}
            >
              {token}
            </div>
        ...
        >
          {tokens.map((_, index) => (
            <path
              ...
              d={
                index % 2
                  ? `M 860 ${170 + index * 230} C 760 ${240 + index * 120}, 810 ${300 + index * 70}, 745 ${330 + index * 115}`
                  : `M 60 ${170 + index * 230} C 240 ${240 + index * 120}, 280 ${300 + index * 70}, 335 ${330 + index * 115}`
              }
              fill="none"
              stroke={PALETTE[index]}
              strokeWidth="2"
...
const SurfaceMorphShot: React.FC<{
  frame: number;
  beat: SkillShowcaseBeat;
  accent: string;
  opacity: number;
  ...
  const names = directionMode
    ? ["Swiss", "RAW", "Nordic", "Neo"]
    : ["官网", "工具", "作品集", "后台"];
  const activeIndex =
    directionMode && p > 0.72
      ? 0
      : Math.min(names.length - 1, Math.floor(p * names.length * 1.35));
  const colors = directionMode
    ? ["#f05a5a", "#9ea7b2", "#7ec8a5", "#20d9e8"]
    : PALETTE;
      ...
      >
        <div
          style={{
            height: 840,
            background:
              activeIndex === 2 && directionMode
                ? "#f1eee6"
                : "rgba(9,13,23,0.96)",
            color: activeIndex === 2 && directionMode ? "#111" : "#fff",
            borderTop: `8px solid ${activeColor}`,
            padding: 42,
            transform: `perspective(900px) rotateY(${Math.sin(frame / 14) * 1.8}deg)`,
            boxShadow: `0 30px 100px rgba(0,0,0,0.52), 0 0 50px ${activeColor}26`,
          }}
        >
          <div style={{ fontFamily: MONO, fontSize: 13, color: activeColor }}>
            LIVE SURFACE / 0{activeIndex + 1}
          </div>
          <div
            style={{
              fontSize: 82,
          ...
          <div
            style={{
              display: "grid",
              gridTemplateColumns:
                activeIndex % 2 ? "1.3fr 0.7fr" : "0.8fr 1.2fr",
              gap: 14,
              marginTop: 150,
            }}
          >
            {Array.from({ length: 4 }).map((_, index) => (
              <div
                key={index}
                style={{
                  height: 120 + (index % 2) * 54,
                  background:
                    activeIndex === 2 && directionMode
                      ? index % 2
                        ? "#dcd8cf"
                        : "#fff"
                      : `${activeColor}${index % 2 ? "12" : "20"}`,
                  borderTop: `3px solid ${activeColor}`,
                  transform: `translateY(${Math.sin(frame / 10 + index) * 8}px)`,
                }}
              />
            ))}
          </div>
          ...
          {names.map((name, index) => (
            <div
              ...
              style={{
                ...
                color:
                  index === activeIndex ? "#fff" : "rgba(255,255,255,0.32)",
                fontWeight: 900,
                fontSize: 17,
              }}
            >
...
const SystemConvergenceShot: React.FC<{
  ...
  const icons = (
    productIcons?.length
      ? productIcons
      : ["impeccable", "frontend-design", "ux-pro", "cloud-design"]
  ) as ProductIconKey[];
  const gather = interpolate(p, [0.06, install ? 0.58 : 0.78], [0, 1], {
    ...clamp,
    easing: easeOut,
  });
  const positions = [
      ...
      >
        ...
        >
          {positions.map(([x, y], index) => (
            <path
              key={index}
              d={`M ${x} ${y} Q 540 ${index % 2 ? 240 : 800}, 540 540`}
              fill="none"
              stroke={PALETTE[index]}
              strokeWidth="3"
          ...
          {Array.from({ length: 12 }).map((_, index) => {
            ...
            return (
              <line
                key={index}
                x1={540 + Math.cos(angle) * 155}
                y1={540 + Math.sin(angle) * 155}
                x2={540 + Math.cos(angle) * 245}
                y2={540 + Math.sin(angle) * 245}
                stroke={index % 2 ? accent : secondary}
                strokeWidth={index % 3 ? 2 : 5}
                opacity={0.55}
              />
            );
          })}
        ...
        {icons.slice(0, 4).map((icon, index) => {
          ...
          return (
            <div
              ...
              style={{
                position: "absolute",
                left: interpolate(gather, [0, 1], [startX, targetX]),
                top: interpolate(gather, [0, 1], [startY, targetY]),
                transform: `translate(-50%, -50%) scale(${interpolate(gather, [0, 1], [0.72, 1])})`,
                opacity: absorbed,
              }}
            >
              <ProductMark
        ...
        <div
          style={{
            ...
            left: 540,
            top: 540,
            width: install ? 210 : 310,
            height: install ? 210 : 310,
            transform: `translate(-50%, -50%) scale(${interpolate(gather, [0, 1], [0.6, 1])})`,
            borderRadius: "50%",
            border: `5px solid ${accent}`,
        ...
        >
          <SemanticIcon
            icon={install ? "plug-zap" : "workflow"}
            color={accent}
            size={install ? 74 : 88}
            framed
          />
...
export const CinematicShot: React.FC<{
  ...
  switch (preset) {
    case "split-wipe":
      return <SplitWipeShot {...shared} secondary={secondary} />;
    case "particle-field":
      return <ParticleFieldShot {...shared} />;
    case "orbital-map":
      return <OrbitalMapShot {...shared} />;
    case "ui-scan":
      return <UiScanShot {...shared} secondary={secondary} />;
    case "material-carousel":
      return <MaterialCarouselShot {...shared} />;
    case "focus-lock":
      return <FocusLockShot {...shared} />;
    case "pipeline-flow":
      return <PipelineFlowShot {...shared} secondary={secondary} />;
    case "token-assembly":
      return <TokenAssemblyShot {...shared} secondary={secondary} />;
    case "surface-morph":
      return <SurfaceMorphShot {...shared} />;
    case "system-convergence":
      return (
        <SystemConvergenceShot
          {...shared}
          secondary={secondary}
    ...
    case "kinetic-type":
      return <KineticTypeShot {...shared} secondary={secondary} />;
  }
};

const BeatRelay: React.FC<{
  beat: SkillShowcaseBeat;
  accent: string;
  progress: number;
...
const SemanticBeatAnimation: React.FC<{
  ...
  if (opacity <= 0) return null;
  const p = phase(frame, beat);
  const enter = interpolate(p, [0, 0.28], [0, 1], {
    ...clamp,
    easing: easeOut,
  });
  ...
  const content = (() => {
      ...
      return (
        <div
          style={{
            ...
            justifyContent: "center",
            gap: 24,
            transform: `translateY(${lift(enter, 24)}px)`,
          }}
        >
          ...
          >
            <div style={{ fontSize: 28, color: "#fff", fontWeight: 950 }}>
              {beat.keyword.replace(number, "").trim() || "语义证据"}
            </div>
            <div
              style={{
                fontFamily: MONO,
            ...
            {rows.slice(0, 3).map((row, index) => (
              <div
                ...
                style={{
                  ...
                  alignItems: "center",
                  gap: 12,
                  color: index === 2 ? "#fff" : "rgba(255,255,255,0.52)",
                  fontSize: 15,
                  fontWeight: 850,
                  transform: `translateX(${interpolate(enter, [0, 1], [54 + index * 18, 0])}px)`,
                }}
              >
                <span
                  style={{
                    fontFamily: MONO,
                    color: index === 2 ? accent : "rgba(255,255,255,.34)",
                  }}
                >
                ...
                <span
                  style={{
                    height: 1,
                    width: index === 2 ? 42 : 24,
                    background: index === 2 ? accent : "rgba(255,255,255,.22)",
                  }}
                />
                <span>{row}</span>
              </div>
      ...
      return (
        ...
        >
          <div
            style={{
              width: 8,
              height: interpolate(enter, [0, 1], [20, 120]),
              background: `linear-gradient(${accent}, transparent)`,
              transform: `rotate(${interpolate(enter, [0, 1], [-22, -8])}deg)`,
            }}
          />
          <div style={{ minWidth: 0 }}>
            <div
        ...
        >
          <div
            style={{
              ...
              justifyContent: "center",
              gap: 14,
              transform: `translateY(${lift(enter, 18)}px)`,
            }}
          >
            <SemanticIcon icon={beat.icon} color={accent} size={30} framed />
            <div
              style={{ fontSize: keywordSize, color: "#fff", fontWeight: 950 }}
            >
              {beat.keyword}
            </div>
          </div>
          ...
          >
            <div
              style={{
                ...
                display: "grid",
                placeItems: "center",
                borderLeft: "5px solid #ff5f7a",
                background:
                  "linear-gradient(90deg, rgba(255,95,122,0.22), rgba(255,95,122,0.07))",
                color: "#ffc0cb",
                fontSize: 16,
                fontWeight: 900,
              }}
            ...
            <div
              style={{
                ...
                background: `linear-gradient(90deg, ${accent}0d, ${accent}25)`,
                color: "#fff",
                fontSize: 16,
                fontWeight: 900,
              }}
          ...
          {Array.from({ length: 10 }).map((_, index) => (
            <div
              ...
              style={{
                ...
                left: "50%",
                top: "50%",
                width: interpolate(enter, [0, 1], [0, 170]),
                height: index % 3 === 0 ? 3 : 1,
                transformOrigin: "0 50%",
                transform: `rotate(${index * 36}deg) translateX(92px)`,
                background: `linear-gradient(90deg, ${index % 2 ? accent : secondary}, transparent)`,
              }}
            />
          ))}
          <div
      ...
      return (
        <div
          style={{
            ...
            top: 72,
            height: 142,
            transform: `rotate(${interpolate(enter, [0, 1], [-5, -1.2])}deg) scale(${interpolate(enter, [0, 1], [1.18, 1])})`,
            display: "flex",
            alignItems: "center",
            justifyContent: "center",
            gap: 22,
    ...
    return (
      ...
      >
        <SemanticIcon icon={beat.icon} color={accent} size={34} />
        <div
          style={{ fontSize: keywordSize + 5, color: "#fff", fontWeight: 950 }}
        >
          {beat.keyword}
        </div>
        <div
      ...
      >
        ...
        <div
          style={{
            ...
            right: 0,
            top: HERO_STAGE_TOP,
            height: HERO_STAGE_HEIGHT * HERO_STAGE_SCALE,
            overflow: "hidden",
          }}
        >
          <div
            style={{
              ...
              transform: `translateX(-50%) scale(${HERO_STAGE_SCALE})`,
