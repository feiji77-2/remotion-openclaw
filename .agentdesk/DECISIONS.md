...
## ADR-20260721-003: Canonical Obsidian Vault Target

- Status: accepted
- Context: A repository-local `kb/` update was previously described as knowledge-base synchronization even though the user's registered Obsidian Vault had not been updated.
- Decision: Every knowledge-base sync must mirror repository `kb/` into Vault ID `0eb4e308bcd29e3e`, rooted at `/Users/macos/dan-koe-brain`; this project uses `/Users/macos/dan-koe-brain/remotion-product-image/kb/` inside that Vault.
- Alternatives considered: Treat repository `kb/` as sufficient; register it as a second Vault; copy pages to the Vault root.
- Consequences: Knowledge sync has two required destinations: the project repository/Gitee and the canonical Obsidian Vault. Completion requires inventory and hash equality. Unrelated changes in the nested Vault repository remain untouched.
- Related task cards: `TC-009-docs-kb-gitee-release`.
